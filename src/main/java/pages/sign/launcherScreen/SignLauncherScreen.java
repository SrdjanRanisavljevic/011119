package pages.sign.launcherScreen;
    ...
    public SignLauncherScreen clickRightArrowButtonToGetToSignInButton() {
        try{
            ...
            MyLogger.log.info("Click right arrow button 3 times to get to Sign In button");
                ...
                boolean condition = new AssertsUtils().isElementVisible(menageDocuments);
                while(!condition){
                    ((AndroidDriver) driver).pressKey(new KeyEvent(AndroidKey.DPAD_RIGHT));
                    Thread.sleep(500);
                    start += 500;
                    condition = new AssertsUtils().isElementVisible(menageDocuments);
                    if(start>end)
